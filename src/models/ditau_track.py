...
base_file = importlib.import_module(f"src.models.base_clustering")
Base = base_file.Base
class Model(Base):
    ...
    def make_graph(self, event, debug=False, **kwargs):
        ...
        for ijet in range(event.nJets):
            ...
            for _ in range(event.JetGhostTrackN[ijet]):
                ...
                nodes.append(utils.get_track_info(event, ghost_track_idx))
        ...
        n_nodes = len(nodes)
        if n_nodes > 15 or nTaus == event.nJets or n_nodes <= 2:
            return None
