...
df = pd.read_csv("../../datasets/SUSY.csv",names=('isSignal','lep1_pt','lep1_eta','lep1_phi','lep2_pt','lep2_eta','lep2_phi','miss_ene','miss_phi','MET_rel','axial_MET','M_R','M_TR_2','R','MT2','S_R','M_Delta_R','dPhi_r_b','cos_theta_r1'))
...
jobn = JOBN
training_size = int(NEVT * 0.7)
testing_size = int(NEVT * 0.3)
shots = 1024
...
feature_map = ZZFeatureMap(feature_dimension=feature_dim, reps=uin_depth)
var_form = EfficientSU2(num_qubits=feature_dim, reps=uvar_depth)
...
plot.savefig('BitCounts_sim_'+backend_name+'_'+str(feature_dim)+'d_'+str(training_size*2)+
             'evt_iter'+str(niter)+'_FMAPuin-depth'+str(uin_depth)+'_uvar-depth'+str(uvar_depth) + '.png')
plot.clf()
